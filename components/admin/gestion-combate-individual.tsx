...
import { Trophy, Play, CheckCircle, Clock, Flame, Plus, Minus, Zap, Sparkles } from "lucide-react"
import { SimuladorKarate, type EventoCombate, type FaltaKarate } from "@/lib/karate-simulador"
...
export function GestionarCombateIndividual({ combate }: { combate: any }) {
  ...
  const [puntos1, setPuntos1] = useState(combate.puntos_atleta1)
  const [puntos2, setPuntos2] = useState(combate.puntos_atleta2)

  const [timeLeft, setTimeLeft] = useState(combate.duracion_minutos * 60)
  const [isTimerRunning, setIsTimerRunning] = useState(false)

  const [simulador] = useState(() => new SimuladorKarate(combate.duracion_minutos))
  const [eventos, setEventos] = useState<EventoCombate[]>([])
    ...
    if (isTimerRunning && timeLeft > 0 && combate.estado === "en_curso") {
      interval = setInterval(() => {
        setTimeLeft((prev) => {
          ...
          if (modoSimulacion && nuevoTiempo > 0) {
            const evento = simulador.simularEvento(combate.duracion_minutos * 60 - nuevoTiempo)
            if (evento) {
              ...
              // Verificar fin por diferencia
              if (simulador.verificarFinPorDiferencia()) {
  ...
  }, [isTimerRunning, timeLeft, combate.estado, modoSimulacion, simulador, combate.duracion_minutos])

  const handleIniciarCombate = async () => {
    setLoading(true)
    const { error } = await supabase.from("combates_individuales").update({ estado: "en_curso" }).eq("id", combate.id)
  ...
  const handleActualizarPuntos = async () => {
    ...
    try {
      const ganadorId = puntos1 > puntos2 ? combate.atleta1.id : puntos2 > puntos1 ? combate.atleta2.id : null

      const { error } = await supabase
        .from("combates_individuales")
        .update({
          puntos_atleta1: puntos1,
          puntos_atleta2: puntos2,
  ...
  return (
    <div className="space-y-6">
      ...
      {modoSimulacion && eventos.length > 0 && (
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-semibold">
                            {evento.atleta === 1 ? combate.atleta1.nombre : combate.atleta2.nombre}
                          </p>
                          <p className="text-sm text-muted-foreground">
                        ...
                        <div className="text-right">
                          <Badge
                            variant={evento.tipo === 'falta' ? "destructive" : "outline"}
                            className="text-lg"
                          >
                            {evento.tipo === 'tecnica' ? '+' : ''}{evento.puntos}
                          </Badge>
                          <p className="text-xs text-muted-foreground mt-1">{formatTime(evento.tiempo)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            >
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                {combate.estado !== "finalizado" && !modoSimulacion && (
            ...
            {combate.estado === "en_curso" && (
              <>
                ...
                <Button
                  ...
                  disabled={loading || (timeLeft > 0 && !simulador.verificarFinPorDiferencia())}
                  size="lg"
                  className="bg-green-600 hover:bg-green-700"
                >
